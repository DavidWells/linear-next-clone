...
function Avatar({ online, name, onClick, avatarUrl }: Props) {
  ...
  if (avatarUrl) {
    avatar = <img src={avatarUrl} alt={name} className="w-4.5 h-4.5 rounded-full" />
  ...
  if (online)
    status = (
      <span className="absolute -right-0.5 -bottom-0.5 w-2 h-2 rounded-full bg-green-500 border border-white" />
    )
  else status = null
